...
class Prevodi(object):
    ...
    def _get_result_links(self, html_page):
        ...
        result = dict()
        self.log.error("html_page: '{0}'".format(html_page))
    ...
    def _get_archive_name(self, adict):
        ...
        if self.HEADER_CONT_DESC.lower() in keys and adict[self.HEADER_CONT_DESC] == 'File Transfer' \
                and self.HEADER_TRANS_ENC.lower() in keys and adict[self.HEADER_TRANS_ENC] == 'binary' \
    ...
    def get_tv_show(self, title):
        ...
        for key, value in list(shows_lower.items()):
    ...
    def get_subtitles(self, season, episode):
        ...
        try:
            self.log.debug("Loaded data from '{0}'".format(self.seasons[season]))
            if 0 <= 1 < len(self.seasons[season][episode]):
